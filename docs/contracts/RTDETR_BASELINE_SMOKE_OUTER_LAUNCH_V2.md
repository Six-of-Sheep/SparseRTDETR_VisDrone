...
single-use pane wrapper path. The wrapper persists its start marker, receipt,
console, exit code, timing, and completion before or around the inner Python
call. It verifies the prepared plan and config file SHA before invoking Python.
...
`classify_outer_evidence` only reads existing evidence. It can report
`OUTER_PREPARED`, `PREFLIGHT_FAILED`, `TMUX_REJECTED`,
`TMUX_ACCEPTED_PANE_NOT_STARTED`, `PANE_STARTED_INNER_NOT_STARTED`,
