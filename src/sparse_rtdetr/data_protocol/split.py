...
class SplitPlan:
    ...
    evaluated_prefix_count: int = 0
    feasible_prefix_count: int = 0
    selected_prefix_length: int = 0


class ProtocolV2ContractError(ProtocolContractError):
    """Raised when the independent feasibility-first V2 planner cannot close."""
...
def _v2_prefix_checks(
    all_groups: tuple[AtomicGroup, ...],
    selected_ids: tuple[str, ...],
    development_sequence_keys: frozenset[str],
    tolerance: float,
) -> tuple[tuple[str, bool], ...]:
    selected = tuple(group for group in all_groups if group.group_id in selected_ids)
    train_core = tuple(group for group in all_groups if group.group_id not in selected_ids)
    ...
    _, train_classes, train_small = _stats(train_core)
    _, confirm_classes, confirm_small = _stats(selected)
    train_total = sum(train_classes)
    confirm_total = sum(confirm_classes)
    non_empty = train_total > 0 and confirm_total > 0
    checks.append(("non_empty_target_statistics", non_empty))
    if non_empty:
        for index in range(10):
            checks.append((
                f"class_{index}_ratio_within_tolerance",
                abs(train_classes[index] / train_total - confirm_classes[index] / confirm_total) <= tolerance,
            ))
        checks.append((
            "coco_small_ratio_within_tolerance",
            abs(train_small / train_total - confirm_small / confirm_total) <= tolerance,
        ))
    else:
        checks.extend((f"class_{index}_ratio_within_tolerance", False) for index in range(10))
        checks.append(("coco_small_ratio_within_tolerance", False))
    return tuple(checks)


def plan_confirmatory_split_v2(
    groups: tuple[AtomicGroup, ...] | list[AtomicGroup],
    *,
    train_image_count: int = 6471,
    target_ratio: float = 0.10,
    ...
    """Select the nearest deterministic prefix only after V2 feasibility filtering."""

    all_groups = tuple(sorted(groups, key=lambda group: group.group_id))
    if not all_groups or train_image_count <= 0:
        raise ProtocolV2ContractError("V2 split planning requires groups and a positive train size")
    candidate = tuple(
        group for group in all_groups
        if group.identity_closed
        and not group.abnormal
    ...
    if not candidate:
        raise ProtocolV2ContractError("V2 split planning has no eligible confirmatory groups")
    ordered = tuple(sorted(
        candidate,
        key=lambda group: hashlib.sha256(f"{salt}\0{seed}\0{group.group_id}".encode("utf-8")).hexdigest(),
    ))
    target = _round_half_up(train_image_count * target_ratio)
    feasible: list[tuple[tuple[object, ...], int, tuple[str, ...], tuple[tuple[str, bool], ...]]] = []
    for length in range(1, len(ordered) + 1):
        prefix = tuple(group.group_id for group in ordered[:length])
        selected_count = sum(group.image_count for group in ordered[:length])
        checks = _v2_prefix_checks(all_groups, prefix, frozenset(development_sequence_keys), tolerance)
        if all(passed for _, passed in checks):
            key = (
                abs(selected_count - target),
                int(selected_count > target),
                selected_count,
                length,
                _group_list_sha(prefix),
            )
            feasible.append((key, selected_count, prefix, checks))
    if not feasible:
        raise ProtocolV2ContractError("no feasible hash prefix satisfies the V2 distribution contract")
    _, selected_count, selected_ids, checks = min(feasible, key=lambda item: item[0])
    selected = tuple(group for group in all_groups if group.group_id in selected_ids)
    train_core = tuple(group for group in all_groups if group.group_id not in selected_ids)
    return SplitPlan(
        train_core_group_ids=tuple(sorted(group.group_id for group in train_core)),
        ...
        evaluated_prefix_count=len(ordered),
        feasible_prefix_count=len(feasible),
        selected_prefix_length=len(selected_ids),
    )
