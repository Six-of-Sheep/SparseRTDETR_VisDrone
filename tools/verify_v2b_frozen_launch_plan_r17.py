#!/usr/bin/env python3
"""Read-only verifier for the concrete REV1 bridge-017 frozen launch plan."""
...
def verify_plan(plan_path: Path, auth_path: Path, repo: Path) -> dict[str,Any]:
    ...
    if plan.get("schema_version") != 2 or plan.get("plan_id") != "P3-V2B-REV1-FROZEN-LAUNCH-BRIDGE017":
        raise ValueError("plan identity/schema mismatch")
    if plan.get("authorization_id") != "smoke-v2b-rev1-s2-r896-bridge-017":
