"""Verify execution-only REV1 smoke launcher and authorization identities."""
...
def verify(repo_root: Path) -> dict[str, Any]:
    root = repo_root / "contracts/v2b/rev001"
    source_path = root / "execution_source_r2.json"
    execution_path = root / "execution_contract_r2.json"
    auth_path = root / "gpu_smoke_authorization_r4.json"
    ...
    for item in source.get("files", []):
        ...
        if not path.is_file():
            errors.append(f"missing execution source: {item['relative_path']}")
            continue
        digest, size = file_sha(path)
        if digest != item.get("sha256") or size != item.get("size_bytes"):
            errors.append(f"execution source identity mismatch: {item['relative_path']}")
    ...
    expected = {
        "smoke_authorization_id": "smoke-v2b-rev1-s2-r896-bridge-004",
    ...
    if scope != {"formal_campaign": False, "gpu_training": True, "max_optimizer_updates": 1, "max_windows": 1}:
