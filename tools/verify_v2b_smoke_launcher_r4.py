#!/usr/bin/env python3
"""Read-only verifier for the REV1 runtime-locator execution revision."""
...
def verify(root: Path, contract_dir: Path, bridge: Path, bridge_manifest: Path, policy_authority: Path) -> dict[str, Any]:
    source_path = contract_dir / "execution_source_r4.json"
    execution = load(source_path)
    if execution["execution_source_id"] != "v2b-exec-004":
    ...
    contract = load(contract_dir / "execution_contract_r4.json")
    if contract["execution_contract_id"] != "v2b-execution-contract-004":
    ...
    auth = load(contract_dir / "gpu_smoke_authorization_r6.json")
    if auth["smoke_authorization_id"] != "smoke-v2b-rev1-s2-r896-bridge-006":
        raise ValueError("unexpected bridge-006 id")
    ...
    if auth["consumed"] is not False or auth["formal_launch_permitted"] is not False or auth["status"] != "PENDING_NOT_EXECUTED":
        raise ValueError("bridge-006 is not pending fail-closed")
