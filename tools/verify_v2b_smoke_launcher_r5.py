#!/usr/bin/env python3
"""Read-only verifier for the REV1 startup-environment execution revision."""
...
def verify(root: Path, contract_dir: Path, bridge: Path, bridge_manifest: Path, policy_authority: Path) -> dict[str, Any]:
    source_path = contract_dir / "execution_source_r5.json"
    execution = load(source_path)
    if execution["execution_source_id"] != "v2b-exec-005":
    ...
    contract = load(contract_dir / "execution_contract_r5.json")
    if contract["execution_contract_id"] != "v2b-execution-contract-005":
    ...
    auth = load(contract_dir / "gpu_smoke_authorization_r7.json")
    if auth["smoke_authorization_id"] != "smoke-v2b-rev1-s2-r896-bridge-007":
    ...
    if auth["consumed"] is not False or auth["formal_launch_permitted"] is not False or auth["status"] != "PENDING_NOT_EXECUTED":
        raise ValueError("bridge-007 is not pending fail-closed")
