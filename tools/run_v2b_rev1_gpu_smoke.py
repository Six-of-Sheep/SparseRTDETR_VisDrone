...
def _execution_contract() -> dict[str, Any]:
    ...
    if contract.get("execution_contract_id") != "v2b-execution-contract-008":
...
def _run_revision_verifier(root: Path) -> dict[str, Any]:
    ...
    command = [
        sys.executable,
        str(REPO / "tools" / "verify_v2b_smoke_launcher_r8.py"),
        "--repo-root", str(REPO),
        "--contract-dir", str(contract_dir),
        "--bridge", str(DERIVED),
        "--bridge-manifest", str(BRIDGE_MANIFEST_PATH),
        "--policy-authority", str(policy_authority),
        "--execution-source", str(REPO / "contracts" / "v2b" / "rev001" / "execution_source_r8.json"),
